...
class CcLabel {
    ...
    private func computeFinalData(
        finalLabels: [Int: Component],
        height: Int,
        width: Int,
        calculateBoundingBoxes: Bool,
        outputData: inout [[Int]]
    ) -> LabelledData {
            ...
            for col in 0 ..< width {
                ...
                if (calculateBoundingBoxes && (label != previousLabelInRow || col == width - 1)) {
                    ...
                    if previousLabelInRow >= 0 {
                        ...
                        let x_start = boundingBoxes[label]!.x_start
                        if col < x_start {
                            boundingBoxes[label]!.x_start = col
                        }
                    }
                }
